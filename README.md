...
2.  **(named volume)** Alternatively we can create a *named volume*. That is a volume managed by
    ...
    ```
    docker run --rm -t -i -v my-named-volume:/myvol
    ```
...
3.  **etherpad-mysql:** the same etherpad appplcation but configured to use a
...
2.  Run `docker-compose up` to build the image and run the container. Note: it
    is not going to work (nginx is not yet installed).
...
7.  **(graceful stop)** Stop your container and restart it in the background.
    ...
    *bash*. When docker stops a container, it sends the termination signall
    ...
    ```
    fig run etherpad cat bin/run.sh | less
    ```
